...
Ca, Pa, epa = diag_F(Hmat, nalpha)
Cb, Pb, epb = diag_F(Hmat, nbeta)
...
for scf_iter in range(maxiter):
    ...
    Ja = einsum('pqrs,rs->pq', Vee, Pa, optimize=True)
    Jb = einsum('pqrs,rs->pq', Vee, Pb, optimize=True)

    Ka = einsum('prqs,rs->pq', Vee, Pa, optimize=True)
    Kb = einsum('prqs,rs->pq', Vee, Pb, optimize=True)

    Fa = Hmat + (Ja + Jb) - Ka
    Fb = Hmat + (Ja + Jb) - Kb

    # build two M's this time
    Ma = dot(Fa, dot(Pa, Smat)) - dot(Smat, dot(Pa, Fa))
    Mb = dot(Fb, dot(Pb, Smat)) - dot(Smat, dot(Pb, Fb))
    diis_r_a = dot(A, dot(Ma, A))
    diis_r_b = dot(A, dot(Mb, A))
    ...
    SCF_E = 0.5*(sum((Pa+Pb)*Hmat) + sum(Pa*Fa) + sum(Pb*Fb)) + E_nuc
